...
export function Navbar() {
  ...
  const { user, loginGoogle, login, logout, switchRole } = useAuth();
  const [showUserMenu, setShowUserMenu] = useState(false);
  const [showGoogleModal, setShowGoogleModal] = useState(false);

  const handleSimulatedGoogleLogin = async (role: 'professor' | 'admin') => {
    const email = role === 'admin' ? 'admin@pedrorizzi.edu.br' : 'elevi.cortelini@pedrorizzi.edu.br';
    const name = role === 'admin' ? 'Coordenador / Admin' : 'Elevi Cortelini';
    const avatar = role === 'admin' 
      ? 'https://images.unsplash.com/photo-1472099645785-5658abf4ff4e?w=150' 
      : 'https://images.unsplash.com/photo-1534528741775-53994a69daeb?w=150';

    await login({ email, name, avatar, role });
          ...
          <div className="bg-white rounded-2xl max-w-md w-full p-6 shadow-2xl border border-slate-100 relative animate-in fade-in zoom-in-95">
            ...
            <div className="text-center mb-6">
              ...
              <p className="text-xs text-slate-500 mt-1">
                Centro Educacional Pedro Rizzi - Agenda de Recursos
              </p>
            </div>
            ...
            <div className="space-y-3">
              <button
                ...
                className="w-full flex items-center justify-center space-x-3 bg-indigo-600 hover:bg-indigo-700 text-white py-3 px-4 rounded-xl font-bold shadow-md shadow-indigo-200 transition-all text-sm"
              >
              ...
              <div className="relative flex py-1 items-center">
                ...
                <span className="flex-shrink mx-3 text-[10px] text-slate-400 font-bold uppercase">ou simular acesso</span>
                <div className="flex-grow border-t border-slate-100"></div>
              </div>

              <button
                onClick={() => handleSimulatedGoogleLogin('professor')}
                className="w-full flex items-center justify-center space-x-2 bg-white border border-slate-300 hover:bg-slate-50 text-slate-700 py-2.5 px-4 rounded-xl font-bold shadow-sm transition-all text-xs"
              >
                <span>Entrar como Professor (Demonstração)</span>
              </button>

              <button
                onClick={() => handleSimulatedGoogleLogin('admin')}
                className="w-full flex items-center justify-center space-x-2 bg-indigo-50 hover:bg-indigo-100 text-indigo-700 py-2.5 px-4 rounded-xl font-bold transition-all text-xs border border-indigo-200"
              >
                <Shield className="w-3.5 h-3.5" />
                <span>Entrar como Administrador (Demonstração)</span>
              </button>
            </div>
